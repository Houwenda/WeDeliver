...
def connectPostgresql():
    conn = psycopg2.connect(database = "postgres", user = "postgres",password = "postgres", host = "127.0.0.1", port="6432" )
...
def insertOrder(sts, did, rid, tm, cg, rw, ps):
...
values(%s, %s, %s, %s, to_timestamp(%s, 'YYYY:MON:DD:HH24:MI:SS'), %s, %s, %s)",(sts, did, rid,tm,cg, rw, ps))
    conn.commit()
    conn.close()
    return 0
